...
describe('TestRegistrationCallbacks', function() {
  ...
  before(function(done) {
    ...
    face = new Face(new UnixTransport());
    ...
    function onRegisterFailed(prefix) {
      ++counter.onRegisterFailedCallCount;
      // Mocha doesn't like "done" being called multiple times, so only call the first time.
      if (counter.onRegisterFailedCallCount == 1)
        done();
    }

    function onRegisterSuccess(prefix, registeredPrefixId) {
      ++counter.onRegisterSuccessCallCount;
      // Mocha doesn't like "done" being called multiple times, so only call the first time.
      if (counter.onRegisterSuccessCallCount == 1)
        done();
    }

    face.registerPrefix
      (new Name("/test/register/callbacks"), null, onRegisterFailed,
       onRegisterSuccess);
  });

  it('RegistrationCallbacks', function() {
    assert.equal(counter.onRegisterSuccessCallCount, 1,
